...
class HyperswarmCLI {
  constructor(options = {}) {
    ...
    this.name = options.name || `peer-${Math.random().toString(36).substr(2, 8)}`
  ...
  setupEventHandlers() {
    ...
    if (process.stdin.isTTY) {
      ...
      process.stdin.on('data', (data) => {
        this.handleUserInput(data.toString().trim())
      })
    }
  }

  handleConnection(conn, info) {
    const peerId = b4a.toString(conn.remotePublicKey, 'hex').substring(0, 8)
